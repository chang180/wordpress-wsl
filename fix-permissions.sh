...
echo "📁 設定 plugins 和 themes 目錄權限（允許 WordPress 和開發者寫入）..."
docker compose exec -u root php chown -R www-data:www-data /var/www/html/wp-content/plugins
docker compose exec -u root php chown -R www-data:www-data /var/www/html/wp-content/themes
docker compose exec -u root php chmod -R 775 /var/www/html/wp-content/plugins
docker compose exec -u root php chmod -R 775 /var/www/html/wp-content/themes
